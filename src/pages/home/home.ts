...
export class HomePage {
  ...
  constructor(public loadingCtrl: LoadingController,private platform: Platform, private geolocation: Geolocation,
    public navCtrl: NavController, public navParams: NavParams) {
      if(navParams.get('posicion')===undefined &&navParams.data!==undefined){
        this.usuario=navParams.data;
        this.locateUser();
      }
      else{
        this.usuario=navParams.get('usuario');
        var coors=navParams.get('posicion');
    ...
    locateUserFavorito(coordenadas){
      ...
      this.geolocation.getCurrentPosition().then((pos) => {
        ...
        this.initMapFav(coordenadas.coords);
      });
    }
    ...
    initMap(posicionu){
      ...
      this.insertarBusqueda(this.map);
      this.insertarParqueaderos(this.map);

    }
    initMapFav(posicionu){
      let latLng = new google.maps.LatLng(posicionu.coords.latitude,posicionu.coords.longitude);
      let mapOptions ={
        center:latLng,
        zoom:15,
      ...
      this.insertarBusqueda(this.map);
      this.insertarParqueaderos(this.map);
      this.insertarFavoritoMapa(this.map,posicionu);

    }

    insertarBusqueda(map){
      //    console.log(map);
      ...
      searchBox.addListener('places_changed', function() {
        ...
        places.forEach(function(place) {
          ...
          '<button ion-button (click)="agregarFav()">'+
          "Agregar a favoritos"+
          "</button>"+
          "</div>";
